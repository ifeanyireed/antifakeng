...
func (a *App) GeneratePrintSheetLocal(config printer.PrintConfig, tokens []string, savePath string) error {
	...
	fmtClean := strings.ToLower(config.Format)
	if fmtClean == "" {
		if strings.HasSuffix(strings.ToLower(savePath), ".png") {
			fmtClean = "png"
		} else if strings.HasSuffix(strings.ToLower(savePath), ".tiff") || strings.HasSuffix(strings.ToLower(savePath), ".tif") {
			fmtClean = "tiff"
		}
	}

	if fmtClean == "png" || fmtClean == "tiff" {
		err = printer.GenerateImageSheet(file, config, tokens)
	} else {
