...
export default function ProducerBatches() {
  ...
  const handleDirectDownload = async () => {
    ...
    try {
      ...
      if (isWails) {
        // Fetch tokens from remote server to render locally
        const tokensResp = await fetch(
          `${apiUrl}/producer/batches/${activePrintBatch.id}/qr-codes`,
          {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          }
        );
        if (!tokensResp.ok) {
          throw new Error(`Failed to fetch batch tokens: ${tokensResp.statusText}`);
        }
        const qrCodes = await tokensResp.json();
        const tokens = qrCodes.map((c: any) => c.token);

        if (!tokens || tokens.length === 0) {
          throw new Error("No tokens found for this batch.");
        }

        // Open native save file dialog
        const savePath = await (window as any).go.main.App.SelectSavePath(
          `antifake-print-${activePrintBatch.batch_code}`,
          fileFormat
        );
        if (!savePath) {
        ...
        // Call Wails local Go bridge to render using desktop's RAM
        const config = {
          batchCode: activePrintBatch.batch_code,
          message: printMessage,
          widthOption: widthParam,
          widthVal: 0,
          format: fileFormat,
          columns: columnsParam,
          labelImage: activePrintBatch.label_image_url || "",
          labelRotation: activePrintBatch.label_rotation || 0,
          qrPosition: activePrintBatch.qr_position || "bottom-right",
          productImage: ""
        };

        await (window as any).go.main.App.GeneratePrintSheetLocal(config, tokens, savePath);
        alert(`Successfully generated and saved print layout to:\n${savePath}`);
        return;
      }
